...
questsRouter.get('/', (req: Request, res: Response) => {
  ...
  if (employer) {
    ...
    params.push((employer as string).trim());
  }

  if (quester) {
    query += ' AND LOWER(quester_wallet) = LOWER(?)';
    params.push((quester as string).trim());
  }
  ...
  const rawQuests = db.prepare(query).all(...params) as any[];
  ...
  const quests = rawQuests.map(q => {
    ...
    const subCount = db.prepare('SELECT count(*) as count FROM submissions WHERE quest_id = ?').get(q.id) as { count: number };
...
questsRouter.get('/:id', (req: Request, res: Response) => {
  const q = db.prepare('SELECT * FROM quests WHERE id = ? OR contract_quest_id = ?').get(req.params.id, req.params.id) as any;
  if (!q) {
...
questsRouter.post('/', (req: Request, res: Response) => {
  const {
    title, description, category, rewardUsdm, proofType,
    secretAnswer, employerWallet, deadlineDays, skillTags,
    employerUsdmBalance, questType, submissionRequirements,
    releaseMode, signedTxHash, signerAddress
  } = req.body;
  ...
  // Balance validation
  const ledger = db.prepare('SELECT * FROM wallet_escrow_ledger WHERE wallet_address = ?').get(employerWallet.toLowerCase()) as any || { total_locked: 0 };
  ...
  const totalCount = db.prepare('SELECT count(*) as count FROM quests').get() as { count: number };
  ...
  `).run(
    ...
    employerWallet.trim(),
    null,
    rewardNum >= 100 ? 'Hard' : rewardNum >= 50 ? 'Medium' : 'Easy',
    JSON.stringify(Array.isArray(skillTags) && skillTags.length ? skillTags : ['Midnight', 'USDM']),
    finalReleaseMode,
    signedTxHash || escrowTxHash,
    escrowTxHash,
    deadline,
...
questsRouter.post('/:id/accept', (req: Request, res: Response) => {
  const { questerWallet } = req.body;
  if (!questerWallet) {
    return res.status(400).json({ success: false, error: 'Quester wallet address is required' });
  }

  const quest = db.prepare('SELECT * FROM quests WHERE id = ? OR contract_quest_id = ?').get(req.params.id, req.params.id) as any;
  if (!quest) {
    return res.status(404).json({ success: false, error: 'Quest not found' });
  }
...
questsRouter.get('/:id/verify', (req: Request, res: Response) => {
  const quest = db.prepare('SELECT * FROM quests WHERE id = ? OR contract_quest_id = ?').get(req.params.id, req.params.id) as any;
  if (!quest) {
    return res.status(404).json({ success: false, error: 'Quest not found' });
  }

  const submissions = db.prepare('SELECT id, quester_wallet, proof_type, proof_hash, status, submitted_at, reviewed_at FROM submissions WHERE quest_id = ?').all(quest.id) as any[];
