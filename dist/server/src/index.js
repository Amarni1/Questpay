...
app.use('/api', bountiesRouter); // supports /api/my-bounties/access
app.use('/api', submissionsRouter);
// Platform Overview Stats (Calculated strictly from real database records)
app.get('/api/stats', (req, res) => {
    const activeQuests = db.prepare("SELECT count(*) as count, coalesce(sum(reward_usdm), 0) as totalLocked FROM bounties WHERE status = 'Open' AND chain_tx_hash IS NOT NULL").get();
    const completedQuests = db.prepare("SELECT count(*) as count, coalesce(sum(reward_usdm), 0) as totalPaid FROM bounties WHERE status = 'Paid'").get();
    const totalBounties = db.prepare("SELECT count(*) as count FROM bounties WHERE chain_tx_hash IS NOT NULL").get();
    res.json({
        success: true,
        stats: {
...
app.get('/api/reputation/:address', (req, res) => {
    ...
    const rep = db.prepare('SELECT * FROM reputation WHERE wallet_address = ?').get(addr);
    const ledger = db.prepare('SELECT * FROM wallet_escrow_ledger WHERE wallet_address = ?').get(addr) || { total_locked: 0, total_earned: 0, total_refunded: 0 };
    res.json({
        success: true,
        reputation: rep ? {
...
app.get('/api/wallet-escrow-metrics/:address', (req, res) => {
    const addr = String(req.params.address).toLowerCase();
    // Sum of USDM locked in active (Open/InProgress) bounties created by this wallet
    const locked = db.prepare("SELECT coalesce(sum(reward_usdm), 0) as total FROM bounties WHERE lower(employer_wallet) = ? AND status IN ('Open', 'InProgress') AND chain_tx_hash IS NOT NULL").get(addr);
    // Sum of USDM earned from approved submissions (Paid bounties where quester is this wallet)
    const earned = db.prepare("SELECT coalesce(sum(b.reward_usdm), 0) as total FROM submissions s JOIN bounties b ON s.bounty_id = b.id WHERE lower(s.quester_wallet) = ? AND b.status = 'Paid' AND s.status = 'Approved'").get(addr);
    // Sum of USDM paid out as employer (bounties this wallet funded that reached Paid status)
    const paidOut = db.prepare("SELECT coalesce(sum(reward_usdm), 0) as total FROM bounties WHERE lower(employer_wallet) = ? AND status = 'Paid' AND chain_tx_hash IS NOT NULL").get(addr);
