...
function initEventListeners() {
  ...
  window.setCreateReleaseMode = setCreateReleaseMode;
  window.cancelSigningCeremony = cancelSigningCeremony;
  window.closeSigningCeremonyAndRefresh = closeSigningCeremonyAndRefresh;
  window.closeMiniSigningOverlay = closeMiniSigningOverlay;
  window.handleEmployerApprove = handleEmployerApprove;
...
function openQuestDrawer(questId) {
  ...
  const isEmployer = connectedWallet.address && quest.employerWallet && quest.employerWallet.toLowerCase() === connectedWallet.address.toLowerCase();

  if (quest.status === 'Paid') {
    actionBtn.disabled = true;
    actionBtn.innerHTML = `<span>✓ Escrow Paid & Completed</span>`;
    actionBtn.onclick = null;
  } else if (isEmployer && quest.status === 'ProofSubmitted') {
    actionBtn.disabled = false;
    actionBtn.innerHTML = `<span>✍️ Sign & Release ${quest.rewardUsdm} USDM Escrow</span>`;
    actionBtn.onclick = () => handleEmployerApprove(quest.id);
  } else if (quest.status === 'ProofSubmitted') {
    actionBtn.disabled = true;
    actionBtn.innerHTML = `<span>⏳ Proof Submitted — Pending Employer Review</span>`;
    actionBtn.onclick = null;
  } else {
    actionBtn.disabled = false;
    actionBtn.innerHTML = `<span>Verify & Release ${quest.rewardUsdm} USDM Escrow</span>`;
    actionBtn.onclick = submitActiveQuestProof;
  }
...
// ---------------------------------------------------------------------------
// Create Quest Modal, Release Mode Selection & Real-Time Balance Validation
// ---------------------------------------------------------------------------
let currentCreateReleaseMode = 'automatic';
let isSigningInProgress = false;
let signingAborted = false;

function setCreateReleaseMode(mode) {
  currentCreateReleaseMode = mode;
  const autoCard = document.getElementById('card-release-auto');
  const manualCard = document.getElementById('card-release-manual');
  const autoRadio = document.getElementById('radio-release-auto');
  const manualRadio = document.getElementById('radio-release-manual');
...
function openCreateQuestModal() {
  ...
  setCreateReleaseMode('automatic');
...
// ---------------------------------------------------------------------------
// 5-STEP CONTRACT SIGNING CEREMONY
// ---------------------------------------------------------------------------

function updateSigningStep(stepIndex, state, title, desc, customHtml = '') {
  // Update step dots 1..5
  for (let i = 1; i <= 5; i++) {
    const dot = document.getElementById(`signing-step-dot-${i}`);
    if (!dot) continue;

    if (i < stepIndex) {
      dot.className = 'signing-step-dot done';
      dot.innerText = '✓';
    } else if (i === stepIndex) {
      if (state === 'error') {
        dot.className = 'signing-step-dot error';
        dot.innerText = '✕';
      } else if (state === 'done') {
        dot.className = 'signing-step-dot done';
        dot.innerText = '✓';
      } else {
        dot.className = 'signing-step-dot active';
        dot.innerText = `${i}`;
      }
    } else {
      dot.className = 'signing-step-dot pending';
      dot.innerText = `${i}`;
    }
  }

  // Update step connectors 1..4
  for (let i = 1; i <= 4; i++) {
    const conn = document.getElementById(`signing-step-conn-${i}`);
    if (!conn) continue;
    if (i < stepIndex) {
      conn.className = 'signing-step-connector done';
    } else if (i === stepIndex - 1 && state === 'active') {
      conn.className = 'signing-step-connector active';
    } else {
      conn.className = 'signing-step-connector';
    }
  }

  // Update step content area
  const titleEl = document.getElementById('signing-step-title');
  const descEl = document.getElementById('signing-step-desc');
  const spinnerEl = document.getElementById('signing-spinner');
  const customEl = document.getElementById('signing-custom-content');

  if (titleEl) titleEl.innerText = title;
  if (descEl) descEl.innerText = desc;
  if (customEl) customEl.innerHTML = customHtml;
...
function openSigningCeremonyModal(questData) {
  signingAborted = false;
  isSigningInProgress = true;

  const modal = document.getElementById('signing-ceremony-backdrop');
  if (modal) modal.classList.add('active');

  // Fill transaction details
  const titleEl = document.getElementById('signing-tx-quest-title');
  const rewardEl = document.getElementById('signing-tx-reward');
  const modeEl = document.getElementById('signing-tx-release-mode');
  const contractEl = document.getElementById('signing-tx-contract');

  if (titleEl) titleEl.innerText = questData.title.length > 28 ? questData.title.slice(0, 28) + '...' : questData.title;
  if (rewardEl) rewardEl.innerText = `${parseFloat(questData.rewardUsdm).toFixed(2)} USDM`;
  if (modeEl) modeEl.innerText = questData.releaseMode === 'automatic' ? '⚡ Automatic (ZK Proof)' : '👤 Manual (Employer Review)';
  if (contractEl) contractEl.innerText = '0x471dfe...e485c';

  const cancelBtn = document.getElementById('btn-cancel-signing');
  const finishBtn = document.getElementById('btn-finish-signing');
  if (cancelBtn) {
    cancelBtn.style.display = 'inline-block';
    cancelBtn.disabled = false;
    cancelBtn.innerText = 'Cancel Ceremony';
  }
  if (finishBtn) finishBtn.style.display = 'none';
}

function cancelSigningCeremony() {
  signingAborted = true;
  isSigningInProgress = false;
  const modal = document.getElementById('signing-ceremony-backdrop');
  if (modal) modal.classList.remove('active');
  showToast('Signing ceremony cancelled', 'info');
}

function closeSigningCeremonyAndRefresh() {
  const modal = document.getElementById('signing-ceremony-backdrop');
  if (modal) modal.classList.remove('active');
  isSigningInProgress = false;
...
async function startSigningCeremony(questData) {
  openSigningCeremonyModal(questData);

  try {
    // -----------------------------------------------------------------------
    // STEP 1: Verify Balance
    // -----------------------------------------------------------------------
    updateSigningStep(1, 'active', 'Step 1/5: Verifying USDM Escrow Balance...', 'Querying wallet unshielded balance and existing contract locks.');
    await sleep(650);
    if (signingAborted) return;

    const userBal = parseFloat(connectedWallet.usdm) || 0;
    const reqReward = parseFloat(questData.rewardUsdm);

    if (userBal < reqReward) {
      updateSigningStep(1, 'error', 'Insufficient USDM Balance', `You have ${userBal.toFixed(2)} USDM available, but this quest requires ${reqReward.toFixed(2)} USDM escrow.`);
      const cancelBtn = document.getElementById('btn-cancel-signing');
      if (cancelBtn) cancelBtn.innerText = 'Close';
      return;
    }

    updateSigningStep(1, 'done', 'Step 1/5: Balance Verified', `${userBal.toFixed(2)} USDM available — sufficient to cover ${reqReward.toFixed(2)} USDM escrow.`);
    await sleep(350);
    if (signingAborted) return;
    ...
    // -----------------------------------------------------------------------
    updateSigningStep(2, 'active', 'Step 2/5: Building Compact Escrow Circuit...', 'Encoding create_quest circuit payload, parameters, and secret commitment.');
    await sleep(750);
    if (signingAborted) return;

    let secretCommitment = null;
    if (questData.proofType === 'AutomatedZkSecret' && questData.secretAnswer) {
      secretCommitment = await computeClientSha256(questData.secretAnswer);
    }

    const circuitPayload = buildCompactCircuitPayload('create_quest', {
      reward_usdm: reqReward,
      proof_type: questData.proofType,
      secret_commitment: secretCommitment ? `0x${secretCommitment}` : null,
      deadline_days: parseInt(questData.deadlineDays) || 3,
      employer: connectedWallet.address
    });

    updateSigningStep(2, 'done', 'Step 2/5: Circuit Constructed', 'Compact 0.17 circuit transition successfully parameterized.');
    await sleep(350);
    if (signingAborted) return;
    ...
    // -----------------------------------------------------------------------
    updateSigningStep(3, 'active', 'Step 3/5: Signing On-Chain Escrow...', 'Awaiting authorization signature from Midnight wallet provider.');
    
    // Attempt signing via Midnight connector
    const signResult = await signContractTransaction(connectedWallet.api, circuitPayload);
    await sleep(800);
    if (signingAborted) return;

    const shortHash = signResult.txHash ? `${signResult.txHash.slice(0, 10)}...${signResult.txHash.slice(-8)}` : 'Signed';
    updateSigningStep(3, 'done', 'Step 3/5: Transaction Signed', `Authorized via ${signResult.method} (${shortHash}).`);
    await sleep(400);
    if (signingAborted) return;
    ...
    // -----------------------------------------------------------------------
    updateSigningStep(4, 'active', 'Step 4/5: Submitting to Midnight Preview...', 'Broadcasting signed circuit transition to ledger validators.');
    ...
    const res = await fetch('/api/quests', {
      ...
      body: JSON.stringify({
        ...questData,
        signedTxHash: signResult.txHash,
        signerAddress: connectedWallet.address,
        employerUsdmBalance: userBal
      })
    });
    ...
    if (!res.ok || !data.success) {
      ...
      throw new Error(data.error || 'Failed to submit escrow to network');
    }

    updateSigningStep(4, 'done', 'Step 4/5: Escrow Locked On-Chain', 'Transaction included in Midnight block state.');
    await sleep(400);
    if (signingAborted) return;
    ...
    // -----------------------------------------------------------------------
    const finalTxHash = data.quest?.escrowTxHash || signResult.txHash;
    const customSuccessHtml = `
      <div class="signing-checkmark">🎉</div>
      <div class="signing-success-hash">
        <span class="hash-label">🔒 Midnight Preview Escrow Tx Hash:</span>
        0x${finalTxHash}
      </div>
      <div style="margin-top: 0.75rem; font-size: 0.78rem; color: var(--gold-secondary);">
        ✓ ${reqReward.toFixed(2)} USDM locked into Compact Escrow • Release Mode: ${questData.releaseMode === 'automatic' ? 'Automatic (ZK)' : 'Manual (Attestation)'}
      </div>
    `;

    updateSigningStep(5, 'done', 'Step 5/5: Bounty Published & Verified!', 'USDM is safely locked in smart contract escrow and visible to all questers.', customSuccessHtml);
    ...
    showToast(`✓ Bounty created and ${reqReward} USDM locked on-chain!`, 'success');
  } catch (err) {
    updateSigningStep(5, 'error', 'Transaction Failed', err.message);
    const cancelBtn = document.getElementById('btn-cancel-signing');
    if (cancelBtn) {
      cancelBtn.innerText = 'Close';
      cancelBtn.disabled = false;
    }
    showToast(`Signing Error: ${err.message}`, 'error');
  } finally {
    isSigningInProgress = false;
  }
}
...
async function handleCreateQuestSubmit(e) {
  ...
  const submissionRequirements = document.getElementById('create-submission-reqs').value.trim();
  const secretAnswer = document.getElementById('create-secret-answer').value.trim();
  ...
  const questData = {
    ...
    secretAnswer,
    releaseMode: currentCreateReleaseMode,
    employerWallet: connectedWallet.address,
    deadlineDays: 3
  };

  // Close create modal and launch the signing ceremony
  closeCreateQuestModal();
  startSigningCeremony(questData);
}

// ---------------------------------------------------------------------------
// Mini Signing Overlay for Employer Manual Approval & Escrow Release
// ---------------------------------------------------------------------------
function openMiniSigningOverlay(amount, recipient) {
  const overlay = document.getElementById('mini-signing-overlay');
  const spinner = document.getElementById('mini-signing-spinner');
  const check = document.getElementById('mini-signing-check');
  const title = document.getElementById('mini-signing-title');
  const desc = document.getElementById('mini-signing-desc');
  const amtEl = document.getElementById('mini-signing-amount');
  const recEl = document.getElementById('mini-signing-recipient');
  const closeBtn = document.getElementById('mini-signing-close-btn');

  if (amtEl) amtEl.innerText = `${amount} USDM`;
  if (recEl) recEl.innerText = recipient ? `${recipient.slice(0, 14)}...${recipient.slice(-8)}` : '--';
  if (spinner) spinner.style.display = 'block';
  if (check) check.style.display = 'none';
  if (title) title.innerText = 'Signing Escrow Release';
  if (desc) desc.innerText = 'Submitting employer_approve_and_release circuit transaction to Midnight Preview...';
  if (closeBtn) closeBtn.style.display = 'none';
  if (overlay) overlay.classList.add('active');
}

function closeMiniSigningOverlay() {
  const overlay = document.getElementById('mini-signing-overlay');
  if (overlay) overlay.classList.remove('active');
}

async function handleEmployerApprove(questId) {
  const quest = allQuests.find(q => q.id === questId);
  if (!quest) return;

  if (!connectedWallet.connected || !connectedWallet.address) {
    showToast('Please connect your employer wallet first', 'error');
    openWalletModal();
    return;
  }

  if (quest.employerWallet.toLowerCase() !== connectedWallet.address.toLowerCase()) {
    showToast('Only the quest creator can approve and release escrow', 'error');
    return;
  }

  openMiniSigningOverlay(quest.rewardUsdm, quest.questerWallet);

  try {
    const circuitPayload = buildCompactCircuitPayload('employer_approve_and_release', {
      quest_id: quest.contractQuestId || quest.id,
    ...
    const res = await fetch(`/api/quests/${quest.id}/employer-approve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        employerWallet: connectedWallet.address,
    ...
    if (title) title.innerText = 'Escrow Released Successfully!';
    if (desc) desc.innerText = `0x${data.payoutTxHash || signResult.txHash} verified on Midnight Preview.`;
    if (closeBtn) closeBtn.style.display = 'block';

    showToast(`✓ Submission approved! ${quest.rewardUsdm} USDM released to quester.`, 'success');
    closeDrawer();
    await refreshWalletBalances();
    await loadPlatformStats();
    await loadQuests();
